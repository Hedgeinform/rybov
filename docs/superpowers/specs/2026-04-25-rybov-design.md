...
- Background fill (когда нет `bg_block`): `#FAFAFA` — **это интринсическая «канва» рыбы**, не page background. Page background приходит от arkadiy-ds (см. 4.5)
- Implicit grid: 10px

**Палитра рыбы НЕ адаптируется под page theme.** Фиш всегда в полноцветной конструктивистской палитре. Это сознательное решение по композиции страницы (см. 4.5: «галерея и экспонаты»).

### 4.5 Page Composition (arkadiy-ds integration)

Web-страница Rybov — **поверхность под брендом Arkadiy Schennikov**. Используется personal design system `arkadiy-ds` (подключена как git submodule в `design-system/`). Принцип композиции — **дисциплинированная DS как галерея, рыбы как экспонаты**: page chrome тихий, артефакт кричащий, контраст работает на композицию, не против.

**Конкретные правила интеграции:**

- **Chrome — целиком из arkadiy-ds:** topbar (Euler `eⁱπ + 1 = 0` в центре + AS-monogram слева, согласно §8.1/§8.2), footer, общая сетка (`--gutter 64px`, `--section-pad 96px`, `--max-width 1320px` — editorial scale, не dense). Никаких rybov-специфичных «брендинговых» дополнений в chrome.
- **Theme — дарк-дефолт + light toggle**, поведение из `arkadiy-ds/components/topbar.html` (§9: `localStorage['theme']` canonical, `prefers-color-scheme` consulted only on first visit).
- **Типографика — strict DS:** Manrope (display + body) + IBM Plex Mono (UI labels only). Никаких системных sans, никакого моно для body.
- **Никакого второго акцента, никаких градиентов, никаких drop-shadow, никаких rounded-cards-with-left-border.** Все правила §3 и §5 DS применяются без исключений.

**Frame для рыбы.** Каждая рыба на странице помещается в `<figure>`:

- 1px `--rule` border вокруг figure (тонкая hairline-рамка, как у §7.4 alerts top/bottom)
- Без fill / без shadow / без tinted background. Page bg показывается через container; рыба **сама** несёт свой `#FAFAFA` canvas внутри SVG
- Padding минимальный (figure плотно облегает SVG, не «дышит»)
- Нет colour-matched accent'ов под рыбой, нет «подсветок в тон». Рыба сама по себе, frame сам по себе

**Caption (museum card pattern).** Под figure — текстовый блок концепта в стиле arkadiy-ds figure-captions:

```
FIG. 2026-04-25
[fish svg]

СЛОВО     मानसून (хинди: монсун — сезон дождей)
РЫБА      Рыба, плывущая против течения собственных слёз
DSL       body: ellipse / blue / left
          tail: triangle / blue / right
          eye: double_circle / front_low
          background_block: yellow / large / [-15, -15]
```

Стилистика caption-блока:

- `FIG. YYYY-MM-DD` сверху figure — IBM Plex Mono uppercase, 11px, letter-spacing 0.5px, `--ink-soft` (DS `--meta-size`)
- `СЛОВО` / `РЫБА` / `DSL` — те же mono uppercase 11px `--ink-soft` labels (DS labels-style)
- Тело caption — Manrope 17px (DS `--body-size`), `--ink`, leading 1.6
- DSL summary — Plex Mono body (~14px), `--ink`, для технического воса — единственное место в layout, где моно используется не для лейбла, и то с обоснованием: DSL **есть** код, его типографика должна это сигнализировать

**Theme-driven tonal contrast.**

- На дарк-теме (`--bg #141110` warm coal): cream `#FAFAFA` canvas рыбы выглядит как яркое полотно на тёмной музейной стене. Сильный contrast, gallery-effect.
- На светлой теме (`--bg #f4ecdd` warm paper): cream `#FAFAFA` рыбы рядом с warm paper — два разных тёплых off-white, тонкая визуальная граница между «artwork canvas» и «page surface». Менее драматично, но всё равно читается.

Оба theme-варианта работают; ни один не требует адаптации SVG.

**Что осталось вне MVP** (parking lot для page composition):
- Decorations brand из §3.5 DS (FFT, Lissajous, phase, ring) в углах rybov-страницы — **не используем**. Рыба сама — единственная декорация на странице. Добавление brand decorations размывает «один экспонат на стене» эффект.
- Dense layout mode (§7.7 DS) — не используем. Rybov — editorial surface, не дашборд.
- Status pills, alerts, tables, charts — не используем. Они в скоупе DS, но на rybov-странице им нечего делать.
...
- **Page chrome через arkadiy-ds** (git submodule в `design-system/`): topbar (Euler + AS-monogram), footer, dark/light theme toggle, типографика Manrope + Plex Mono (**self-hosted woff2** из `design-system/arkadiy-ds/fonts/` — без Google Fonts CDN), токены через CSS custom properties
...
- **Repo path под hedgeinform:** конкретный URL-prefix для web (влияет на Astro `base` config)
- **arkadiy-ds integration в Astro:** копирование `design-system/arkadiy-ds/{tokens.css, typography.css, fonts/}` в `web/public/arkadiy-ds/` через build-prep step (fonts папка обязательна — woff2 загружаются через `@font-face` в typography.css по относительным путям), embedding `topbar.html` + `footer.html` как Astro components, inheritance theme-toggle behaviour без модификаций

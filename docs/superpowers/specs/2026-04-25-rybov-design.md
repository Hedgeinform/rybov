...
Системные промпты каждой стадии — стабильны, кэшируются (`cache_control: ephemeral`). Покрывает ~80% input-токенов. Не критично экономически (объём малый), но правильная гигиена.
...
| 3. Секреты в credential store | `ANTHROPIC_API_KEY`, TG bot token — env-vars, не в коде |

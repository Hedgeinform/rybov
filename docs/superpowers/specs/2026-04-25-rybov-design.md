...
- **Page chrome через arkadiy-ds** (git submodule в `design-system/`): topbar (Euler + AS-monogram), footer, dark/light theme toggle, типографика Manrope + Plex Mono, токены через CSS custom properties
...
- **arkadiy-ds integration в Astro:** загрузка `design-system/arkadiy-ds/tokens.css` + `typography.css` в Astro Layout, embedding `topbar.html` + `footer.html` как статичных HTML-фрагментов, подключение Manrope + IBM Plex Mono через Google Fonts (per DS README), inheritance theme-toggle behaviour без модификаций

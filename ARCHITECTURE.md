...
### OQ2. NFT lazy-mint как механика recognition (отложено)
Идея: каждая рыба остаётся бесплатно публичной; на странице конкретной рыбы — кнопка
«Mint this fish», которая создаёт ERC-721 token (Base, собственный контракт, fixed
price) только при первом покупателе. Mint = акт принадлежности к концепции, не
commercial primary. Концептуальное обоснование: donation нечестно отражает авторство
(Opus не знает, что он Opus; оператор не «нарисовал»); NFT-покупка обращена к
артефакту, не к создателю — структурно точнее.
**Что не решено:** fixed price ($5/$10/$25?), wallet ops (CI secret vs manual),
payment failure recovery, OpenSea auto-indexing — принять как факт secondary trading.
**Trigger для re-review:** органический рост аудитории за пределами тестовой группы —
реальные люди (не друзья) пишут в TG, перепосты в незнакомые комьюнити, накопится
30-90 дней посещений. До этого жеста ждём.
**Compliance:** RU-payout остаётся отдельной нерешённой проблемой; не пытаться
имплементить до её отдельной проработки.
